...
const (
	...
	NumberOfRetryWithWriteError   = 3
	IntervalOfRetryWithWriteError = 1 * time.Second
)
	...
	for _, owner := range shard.Owners {
		if owner.NodeID == w.Node.ID {
			...
			if err == tsdb.ErrShardNotFound {
				...
				if err != nil {
					w.Logger.Info("Write failed", zap.Uint64("shard", shard.ID), zap.String("node", "local"), zap.Error(err))
					atomic.AddInt64(&w.stats.WriteErr, 1)
					return err
				}
			}
			...
			atomic.AddInt64(&w.stats.PointWriteReq, int64(len(points)))
			for i := 1; i <= NumberOfRetryWithWriteError; i++ {
				err := w.ProxyStore.WriteToShard(shard.ID, owner.NodeID, database, retentionPolicy, points)
				if err != nil {
					w.Logger.Error("Write failed", zap.Uint64("shard", shard.ID), zap.Uint64("node", owner.NodeID), zap.Int("retry", i), zap.Error(err))
					if i == NumberOfRetryWithWriteError {
						w.Logger.Error("Result: Write failed", zap.Uint64("shard", shard.ID), zap.Uint64("node", owner.NodeID), zap.Error(err))
						return errors.New(fmt.Sprintf("write to shardID %d with nodeID %d failed because %s", shard.ID, owner.NodeID, err.Error()))
					}
					time.Sleep(IntervalOfRetryWithWriteError)
				} else {
					break
				}
			}
...
// execute this when local rpc server listen writeToShard request
func (w *PointsWriter) RpcWriteToShard(shardID uint64, database string, retentionPolicy string, points []models.Point) error {
	...
	if err == tsdb.ErrShardNotFound {
		...
		if err != nil {
			w.Logger.Info("Write failed", zap.Uint64("shard", shardID), zap.String("node", "local"), zap.Error(err))
