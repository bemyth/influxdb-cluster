...
func (e *StatementExecutor) ExecuteStatement(stmt influxql.Statement, ctx *query.ExecutionContext) error {
	...
	case *influxql.ShowMeasurementsStatement:
		...
		//TODO                 Undone (返回表数量，底层实现决定结果无法合并，可以不考虑该功能的集群化实现)
	...
	case *influxql.ShowRetentionPoliciesStatement:
		...
		//TODO                 Undone (返回series数量，底层实现决定结果无法合并，可以不考虑该功能的集群化实现)
	...
	case *influxql.ShowShardGroupsStatement:
		...
		//TODO                 Undone (show stats 显示节点状态，无需更改)
	...
	case *influxql.SetPasswordUserStatement:
		...
		//TODO                 Undone (节点间需要能够互相访问taskmanager)
...
func (e *StatementExecutor) executeDeleteSeriesStatement(stmt *influxql.DeleteSeriesStatement, database string) error {
	...
	//Cluster delete the series
	return e.ProxyStore.DeleteSeries(database, stmt.Sources, stmt.Condition)
}
...
func (e *StatementExecutor) executeDropDatabaseStatement(stmt *influxql.DropDatabaseStatement) error {
	...
	//	return err
	//}
	//Cluster delete the database
	if err := e.ProxyStore.DeleteDatabase(stmt.Name); err != nil {
		return err
	}
...
func (e *StatementExecutor) executeDropMeasurementStatement(stmt *influxql.DropMeasurementStatement, database string) error {
	...
	// Cluster drop the measurement
	return e.ProxyStore.DeleteMeasurement(database, stmt.Name)
}
...
func (e *StatementExecutor) executeDropShardStatement(stmt *influxql.DropShardStatement) error {
	...
	//	return err
	//}
	// Cluster delete the shard.
	if err := e.ProxyStore.DeleteShard(stmt.ID); err != nil {
		return err
	}
...
func (e *StatementExecutor) executeDropRetentionPolicyStatement(stmt *influxql.DropRetentionPolicyStatement) error {
	...
	//	return err
	//}
	// Cluster drop the retention policy
	if err := e.ProxyStore.DeleteRetentionPolicy(stmt.Database, stmt.Name); err != nil {
		return err
	}
...
func (e *StatementExecutor) executeShowMeasurementsStatement(q *influxql.ShowMeasurementsStatement, ctx *query.ExecutionContext) error {
	...
	names, err := e.ProxyStore.MeasurementNames(ctx.Authorizer, q.Database, q.Condition)
...
func (e *StatementExecutor) executeShowTagKeys(q *influxql.ShowTagKeysStatement, ctx *query.ExecutionContext) error {
	...
	// Cluster Show TagKeys
	tagKeys, err := e.ProxyStore.TagKeys(ctx.Authorizer, shardIDs, cond)
	if err != nil {
		return ctx.Send(&query.Result{
			Err: err,
		})
...
func (e *StatementExecutor) executeShowTagValues(q *influxql.ShowTagValuesStatement, ctx *query.ExecutionContext) error {
	...
	//Cluster Show TagValues
	tagValues, err := e.ProxyStore.TagValues(ctx.Authorizer, shardIDs, cond)
	if err != nil {
...
type TSDBStore interface {
	...
	//TODO(shan,jt for cluster) add ShardGroup to implement shardMapper in selectStatement
