...
func init() {
	...
	tests["drop_and_recreate_database"] = Test{
		...
		queries: []*Query{
			...
			&Query{
				name:    "Re policy",
				command: ` POLICY rp0 ON db0 DURATION 365d REPLICATION 1 DEFAULT`,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
	...
	tests["retention_policy_commands"] = Test{
		...
		queries: []*Query{
			&Query{
				name:    " policy with invalid name should return an error",
				command: ` POLICY "." ON db0 DURATION 1d REPLICATION 1`,
				exp:     `{"results":[{"statement_id":0,"error":"invalid name"}]}`,
				once:    true,
			},
			&Query{
				name:    " policy should succeed",
				command: ` POLICY rp0 ON db0 DURATION 1h REPLICATION 1`,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
			...
			&Query{
				...
				command: ` POLICY rp2 ON db0 DURATION 1h REPLICATION 1`,
				exp:     `{"results":[{"statement_id":0}]}`,
				once:    true,
			},
			&Query{
