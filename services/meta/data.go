...
const (
	...
	MinRetentionPolicyDuration = time.Second
)
...
func (data *Data) CreateShardGroup(database, policy string, timestamp time.Time) error {
	...
	for i := range sgi.Shards {
		...
		for j := 0; j < replicaN; j++ {
			nodeId := data.DataNodes[rand%len(data.DataNodes)].ID
...
func (data *Data) CloneDataNodes() []NodeInfo {
	if len(data.DataNodes) == 0 {
		return []NodeInfo{}
	}
	other := make([]NodeInfo, len(data.DataNodes))
	for i := range data.DataNodes {
		other[i] = data.DataNodes[i].clone()
	}
	return other
}
func (data *Data) UpdateDataNode(ni NodeInfo) error {
	exist := false
	for _, node := range data.DataNodes {
		if node.ID == ni.ID {
			node.Host = ni.Host
			node.TCPHost = ni.TCPHost
			exist = true
		}
	}
	if !exist {
		data.DataNodes = append(data.DataNodes, ni)
	}
	return nil
}
...
func (data *Data) marshal() *internal.Data {
	pb := &internal.Data{
		...
		//PreVersion:      proto.Int64(data.PreVersion),
	...
	//for i := range data.DataNodes {
	//	pb.DataNodes[i] = data.MetaNodes[i].marshal()
	//}
...
func (data *Data) unmarshal(pb *internal.Data) {
	...
	//data.PreVersion = pb.GetPreVersion()
}
...
// unmarshal deserializes from a protobuf representation.
func (ui *UserInfo) Unmarshal(bauth []byte) error {
	pb := &internal.UserInfo{}
	err := proto.Unmarshal(bauth, pb)
	if err != nil {
		return err
	}
	ui.Name = pb.GetName()
...
func EncodeAuthorizer(auth query.Authorizer) ([]byte, error) {
	var bauth []byte
	switch auth.(type) {
	case *UserInfo:
		{
			pauth := auth.(*UserInfo).Marshal()
			tmp, err := proto.Marshal(pauth)
			if err != nil {
				return nil, err
			}
			bauth = tmp
...
func DecodeAuthorizer(bauth []byte) (query.Authorizer, error) {
	if bauth == nil || len(bauth) == 0 {
		return query.OpenAuthorizer, nil
	}
	ui := &UserInfo{}
	if err := ui.Unmarshal(bauth); err != nil {
		return nil, err
	}
	return ui, nil
}
