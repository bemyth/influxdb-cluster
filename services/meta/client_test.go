...
func TestClient_AcquireLease(t *testing.T) {
	cli, err := clientv3.New(clientv3.Config{
		Endpoints: []string{"172.16.41.15:2379"},
	})
	rst := []byte{}
	resp, err := cli.Get(context.Background(), "/influxdb/cachedata")
	...
	data := meta.Data{}
	data.UnmarshalBinary(rst)
	//fmt.Println(data.DataNodes)
	for _, sg := range data.Database("mydb").RetentionPolicy("autogen").ShardGroups {
		fmt.Println(sg)
	}
}
