...
func (itr *bufIntegerIterator) NextBatch() ([]*IntegerPoint, error) {
...
func (itr *integerMergeIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
...
func (itr *integerSortedMergeIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	p, err := itr.pop()
	results = append(results, p)
	return results, err
...
func (itr *integerParallelIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	v, ok := <-itr.ch
	if !ok {
...
func (itr *integerLimitIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	for {
...
func (itr *integerFillIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	if !itr.init {
...
func (itr *integerIntervalIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	p, err := itr.input.Next()
	if p == nil || err != nil {
		return results, err
...
func (itr *integerInterruptIterator) NextBatch() ([]*IntegerPoint, error) {
...
func (itr *integerCloseInterruptIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	p, err := itr.input.Next()
	if err != nil {
...
func (itr *integerReduceFloatIterator) NextBatch() ([]*FloatPoint, error) {
...
func (itr *integerStreamFloatIterator) NextBatch() ([]*FloatPoint, error) {
	results := make([]*FloatPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *integerReduceIntegerIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *integerStreamIntegerIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *integerReduceUnsignedIterator) NextBatch() ([]*UnsignedPoint, error) {
...
func (itr *integerStreamUnsignedIterator) NextBatch() ([]*UnsignedPoint, error) {
	results := make([]*UnsignedPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *integerReduceStringIterator) NextBatch() ([]*StringPoint, error) {
...
func (itr *integerStreamStringIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *integerReduceBooleanIterator) NextBatch() ([]*BooleanPoint, error) {
...
func (itr *integerStreamBooleanIterator) NextBatch() ([]*BooleanPoint, error) {
	results := make([]*BooleanPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *integerIteratorMapper) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
...
func (itr *integerFilterIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	for {
		p, err := itr.input.Next()
		if err != nil || p == nil {
...
func (itr *integerDedupeIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	for {
		// Read next point.
		p, err := itr.input.Next()
...
func (itr *integerReaderIterator) NextBatch() ([]*IntegerPoint, error) {
