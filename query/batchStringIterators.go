...
func (itr *bufStringIterator) NextBatch() ([]*StringPoint, error) {
...
func (itr *stringMergeIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	itr.mu.RLock()
...
func (itr *stringSortedMergeIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	p, err := itr.pop()
	results = append(results, p)
	return results, err
...
func (itr *stringParallelIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	v, ok := <-itr.ch
	if !ok {
...
func (itr *stringLimitIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	for {
...
func (itr *stringFillIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	if !itr.init {
...
func (itr *stringIntervalIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	p, err := itr.input.Next()
	if p == nil || err != nil {
		return results, err
...
func (itr *stringInterruptIterator) NextBatch() ([]*StringPoint, error) {
...
func (itr *stringCloseInterruptIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	p, err := itr.input.Next()
	if err != nil {
...
func (itr *stringReduceFloatIterator) NextBatch() ([]*FloatPoint, error) {
...
func (itr *stringStreamFloatIterator) NextBatch() ([]*FloatPoint, error) {
	results := make([]*FloatPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *stringReduceIntegerIterator) NextBatch() ([]*IntegerPoint, error) {
...
func (itr *stringStreamIntegerIterator) NextBatch() ([]*IntegerPoint, error) {
	results := make([]*IntegerPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *stringReduceUnsignedIterator) NextBatch() ([]*UnsignedPoint, error) {
...
func (itr *stringStreamUnsignedIterator) NextBatch() ([]*UnsignedPoint, error) {
	results := make([]*UnsignedPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *stringReduceStringIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *stringStreamStringIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *stringReduceBooleanIterator) NextBatch() ([]*BooleanPoint, error) {
...
func (itr *stringStreamBooleanIterator) NextBatch() ([]*BooleanPoint, error) {
	results := make([]*BooleanPoint, 0)
	// Calculate next window if we have no more points.
	if len(itr.points) == 0 {
		var err error
...
func (itr *stringIteratorMapper) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
...
func (itr *stringFilterIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	for {
		p, err := itr.input.Next()
		if err != nil || p == nil {
...
func (itr *stringDedupeIterator) NextBatch() ([]*StringPoint, error) {
	results := make([]*StringPoint, 0)
	for {
		// Read next point.
		p, err := itr.input.Next()
...
func (itr *stringReaderIterator) NextBatch() ([]*StringPoint, error) {
