...
import (
	...
	"github.com/influxdata/influxdb/services/rpc"
)
...
func NewServer(c *Config, buildInfo *BuildInfo) (*Server, error) {
	...
	//TODO(shan.jt for cluster) load nodeinfo and inject to somewhere needed 2018.8.18-1 --modify
	nodeInfo := influxdb.NewNode(c.Meta.Dir)
	id, err := strconv.ParseUint(c.RaftConfig.LocalID, 10, 64)
	nodeInfo.ID = id
	nodeInfo.RpcAddress = c.RaftConfig.RpcAddress
	nodeInfo.Host = c.RaftConfig.RaftAddress

	//TODO(shan.jt for unnecessary) we don't need local raftdb 2018.8.18-2 --delete
	//if err := raftDBExists(c.Meta.Dir); err != nil {
	//	return nil, err
	//}

	//TODO(shan.jt for cluster) NewRpcServer
	rpcs := rpc.NewRpcServer(nodeInfo.RpcAddress)
	...
	s := &Server{
		...
		//TODO(shan.jt for cluster) modify meta and add rpc
	...
	//TODO(shan.jt for cluster) inject nodeinfo to metaclient --add
	s.MetaClient.Node = nodeInfo

	//TODO(给store赋予网络传输的接口，以便进行步进处理)
	store := meta.NewStore(c.RaftConfig)
	store.Transmitter = s.RpcServer.Transmitter
	s.MetaClient.Store = store
	...
	//TODO(shan.jt for cluster) new cluster
	s.Cluster = coordinator.NewCluster()
	s.Cluster.MetaClient = s.MetaClient
	s.Cluster.TSDBStore = s.TSDBStore
	s.Cluster.Transmitter = s.RpcServer.Transmitter
	s.Cluster.WithLogger(s.Logger)
	...
	//TODO(shan.jt for cluster) inject nodInfo to PointsWrite 2018.8.18-3 --add
	s.PointsWriter.Node = nodeInfo
	//TODO() 2018.8.21 -add
	s.PointsWriter.ProxyStore = s.Cluster

	s.RpcServer.LocalWriter = s.PointsWriter
	s.RpcServer.LocalExecutor = s.Cluster
	s.RpcServer.LocalRaftStore = store
	...
	s.QueryExecutor.StatementExecutor = &coordinator.StatementExecutor{
		...
		//	MetaClient: s.MetaClient,
		//	PROXYStore:  s.MetaClient,                      //coordinator.LocalTSDBStore{Store: s.TSDBStore},
		//},
		Monitor:           s.Monitor,
...
func (s *Server) Open() error {
	...
	//TODO(shan.jt for raft) open rpc server first
	//TODO(shan.jt for cluster) open rpcsever --add withlog --add
	if err := s.RpcServer.Open(); err != nil {
		return fmt.Errorf("open rpc server: %s", err)
	}
	s.RpcServer.WithLogger(s.Logger)

	if err := s.MetaClient.Open(); err != nil {
		return fmt.Errorf("open metaclient: %s", err)
	}
	// Open TSDB store.
